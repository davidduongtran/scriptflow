// ═══════════════════════════════════════════════════════════════════════════════
// FINRICH SCRIPT v1.6.4
// YouTube Script Generation + VEO 3 Prompt Generation
// ═══════════════════════════════════════════════════════════════════════════════

console.log('🚀 FinRich Script v1.6.4 Loading...');
...
const CLONE_MODE_CONFIG = {
  ...
  '95': {
    ...
    instruction: 'Preserve structure and emotional arc, rewrite all content with fresh examples'
  }
};

// ═══════════════════════════════════════════════════════════════════════════════
// VISUAL RENDERING SYSTEM v2.0 - Complete Style Specification
// 22 Visual Styles with Technical DNA, 14 Niches, Auto-Detection
// ═══════════════════════════════════════════════════════════════════════════════
...
function autoApplyStyleSuggestion(scriptContent) {
  ...
  if (suggestion.confidence >= 50 && !state.styleManuallySet) {
    ...
    if (styleSelect) {
      ...
      updateStylePreview();
      console.log('🎨 AI Style Suggestion:', suggestion.suggestedStyle, `(score: ${suggestion.confidence})`);
      veoLog(`🎨 AI suggested: ${suggestion.suggestedStyle} (${suggestion.confidence}% confidence)`, 'info');
    }
  }
...
function updateStylePreview() {
  ...
  const technicalDNAEl = document.getElementById('styleTechnicalDNA');
  const colorPaletteEl = document.getElementById('styleColorPalette');
  const bestForEl = document.getElementById('styleBestFor');
  ...
  if (technicalDNAEl) technicalDNAEl.textContent = spec.technicalDNA;
  if (colorPaletteEl) colorPaletteEl.textContent = spec.colorPalettes[niche] || spec.colorPalettes.finance;
  if (bestForEl) bestForEl.textContent = `Best for: ${spec.bestFor}`;
}
...
/**
 * Build complete style configuration object
 */
...
// Note: VISUAL_STYLE_SPECS, NICHE_DATABASE, and helper functions defined at top of file
// ──────────────────────────────────────────────────────────────────────────────
...
// ═══════════════════════════════════════════════════════════════════════════════
// NICHE DATABASE - 14 Niches with Rankings and Style Recommendations
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// HELPER FUNCTIONS - Style Anchor, Auto-Detection, UI Updates
// ═══════════════════════════════════════════════════════════════════════════════
...
function buildMasterPrompt(topic, numPrompts, videoStyle, dialogueLanguage, subtitles, startSceneNum = 1) {
  ...
  // ===== STYLE ANCHOR INTEGRATION (Visual Rendering System v2.0) =====
  const niche = document.getElementById('contentNiche')?.value || 'finance';
  ...
  try {
    ...
    await chrome.storage.local.set({
      ...
      veoPromptsMetadata: {
        ...
        sourceVersion: 'FinRich Script v1.6.4'
      }
    });
...
---
*Generated by FinRich Script v1.6.4*
`;
...
function exportPostProduction() {
  ...
  const jsonContent = {
    meta: { version: baseFilename, exportedAt: new Date().toISOString(), exportedBy: 'FinRich Script v1.6.4' },
...
---
Exported by FinRich Script v1.6.4
`;
}
